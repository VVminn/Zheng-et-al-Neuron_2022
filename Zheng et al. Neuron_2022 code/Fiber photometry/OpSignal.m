...
% Last Modified by GUIDE v2.5 22-Jun-2018 09:28:27
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
[filename, pathname] = uigetfile({'*.CSV';'*.tdms'}, 'Open data file');%获取选择的文件名称与路径信息
handles.file_path_and_name = fullfile(pathname, filename);%获取选择文件的完整文件名，路径加上名称
[~,~,file_part]=fileparts(handles.file_path_and_name);
if strcmp(file_part,'.csv')
    data = csvread(handles.file_path_and_name);
elseif strcmp(file_part,'.tdms')
    flie = TDMS_readTDMSFile(handles.file_path_and_name);
    data=cat(1,flie.data{3:end})';
end 
if filename == 0
    return;
end

set(handles.edit_file, 'String', handles.file_path_and_name);%将handles中edit_file设置为路径名称
...
% data_cue=smooth(handles.data1,10);
% data_cue(data_cue<1)=0;
% data_cue(data_cue>=1)=1;
% handles.current_cue=data_cue;
...
interval = 1/str2num(get(handles.TB_SampleRate,'String'));
handles.TB_SampleRate=str2num(get(handles.TB_SampleRate,'String'));
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% if bin<interval
%     bin=interval;
...
for i=1:trialnum
    if upindex(i) < basal_time*handles.TB_SampleRate
        upindex(i)=0;
...
for i =1:trialnum
    if upindex(i)+odor_time*handles.TB_SampleRate>length
        upindex(i)=0;
        trialnumtemp = trialnumtemp-1;
    end
end
...
trigger1_times = trigger_times_pretreatment(upindex,interval,trial_from,sti1_number,handles.edit_sti1);
...
% string=strrep(handles.file_path_and_name,'csv','mat');
% string=string(1:end-4);
% string_file=[string,'_',num2str(trial_from),'.mat'];
% save(string_file,'times','psth1','psth1_mean','psth1_sem');
...
%             handles.upindex=1+find(diff(handles.current_cue)==1)'-4;
        ...
        case 2
%             data_cue=smooth(handles.data2,10);
%             data_cue(data_cue<1)=0;
%             data_cue(data_cue>=1)=1;
%             handles.current_cue=data_cue;
%             handles.upindex=1+find(diff(handles.current_cue)==1)'-4;
        ...
        case 3
%             data_cue=smooth(handles.data3,10);
%             data_cue(data_cue<1)=0;
%             data_cue(data_cue>=1)=1;
%             handles.current_cue=data_cue;
%             handles.upindex=1+find(diff(handles.current_cue)==1)'-4;
...
psth_mean=Data_mean;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%求取psth_sem变量%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
psth_sem = std(psth_ALL)/(N-1)^0.5;
save 'ALLdata.mat' psth_ALL psth_mean psth_times psth_sem;
...
load('ALLdata.mat');
figure;
drawErrorLine(psth_times,psth_mean,psth_sem,'red',0.5);
xlabel('Time (s)');
figure;
imagesc(psth_times,1:1:size(psth_ALL,1),psth_ALL);
xlabel('Time (s)');
...
% --- Executes on button press in Event_time.
function Event_time_Callback(hObject, eventdata, handles)
% hObject    handle to Event_time (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
event_time=str2num(get(handles.edit_event,'String'));
count_trialnum=length(event_time);
handles.trialnum=count_trialnum;
rate=str2num(get(handles.TB_SampleRate,'String'));
event_time=round(event_time*rate);
set(handles.edit_sti1, 'String', num2str(handles.trialnum));
handles.upindex=event_time;
guidata(hObject,handles);
